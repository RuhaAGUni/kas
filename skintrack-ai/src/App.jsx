...
function getIndicatorCards(result) {
  const indicatorLabels = {
    redness: 'Redness',
    inflamedLesions: 'Inflamed lesions',
    comedones: 'Comedones',
    scarringRisk: 'Scarring risk / post-acne marks',
  }

  if (result.indicatorStatus) {
    return Object.entries(indicatorLabels).map(([key, label]) => {
      const indicator = result.indicatorStatus[key]

      return {
        confidence: indicator?.confidence,
        evidence: indicator?.evidence || 'No supporting YOLO evidence.',
        key,
        label,
        prominent: indicator?.status === 'prominent',
        status: indicator?.status || 'not prominent',
      }
    })
  }

  const simulatedIndicators = result.detectedIndicators || []
  ...
  return Object.entries(indicatorLabels).map(([key, label]) => {
    const indicatorNames =
      key === 'inflamedLesions'
        ? ['inflamed lesions']
        : key === 'scarringRisk'
          ? ['scarring risk', 'post-acne marks']
          : [key]
    const prominent = indicatorNames.some((indicatorName) =>
      simulatedIndicators.includes(indicatorName),
    )

    return {
      confidence: prominent ? result.confidence : 0,
      evidence: prominent
        ? 'Prototype fallback suggested this indicator.'
        : 'Prototype fallback did not flag this indicator.',
      key,
      label,
      prominent,
      status: prominent ? 'prominent' : 'not prominent',
    }
  })
}
  ...
  return (
    <div className="analysis-results">
      ...
      <div className="overlay-note">
        ...
        <span>
          {result.source === 'real-yolo-model'
            ? 'Bounding boxes are generated by the YOLO model. Indicator interpretation is prototype support only.'
            : 'AI Overlay is simulated for prototype demonstration.'}
        </span>
      </div>

      <div className="indicator-grid">
        {getIndicatorCards(result).map((indicator) => (
          <div
            className={
              indicator.prominent ? 'indicator-chip detected' : 'indicator-chip'
            }
            key={indicator.key}
          >
            <CheckCircle size={16} />
            ...
            <strong>
              {indicator.status}
              {indicator.confidence ? ` · ${indicator.confidence}%` : ''}
            </strong>
          </div>
        ))}
      </div>
